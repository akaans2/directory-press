...
switch($_GET['action']){
	...
	case "UserActionForm": {
		...
		if($_GET['act'] == "login"){
			...
			if ( is_wp_error($user) ){
				...
				print "<a href='' onClick=\"jQuery('#loginform').show();\">click here to try again</a>";
			
			}else{
			
				print "Welcome back ".$user->display_name."<br/><br/>";
				
				print "<p class='CheckoutBtn'><a href='javascript:void(0);' onClick=\"jQuery('#step1box').hide();jQuery('#step2box').show();;\">click here to continue</a></p>";
	...
	if (empty($array['total'])) {
 	  ...
 	  $my_post = array();
	  $my_post['post_title'] 	= $_GET['title']." at ".$dd1[0];
	  $my_post['post_content'] 	= $_GET['desc'];
	  $my_post['post_excerpt'] 	= $_GET['desc'];
...
case "catprice": {
   ...
   if($price == ""){ $price=0; }
	...
	if(in_array($_GET['current'],$state_included)){
			...
			if(isset($_GET['ship'])){
			$string = '<p class="f_half left"><label>'.$tt.'</label><br /><select name="shipping[state]" class="short">';
			}else{
			$string = '<p class="f_half left"><label>'.$tt.'</label><br /><select name="form[state]" class="short">';
			}
		...
		if(isset($_GET['ship'])){
		print  '<p class="f_half left"><label class="col1">'.$tt.'</labe<br /><input type="text" name="shipping[state]" class="short" size="35" maxlength="100"/></p>';
		}else{
		print  '<p class="f_half left"><label class="col1">'.$tt.'</labe<br /><input type="text" name="form[state]" class="short" size="35" maxlength="100"/></p>';
		}
...
if(is_numeric($_GET['d'])){
	...
	$PASSCODE = '<h1>%title%</h1>
			...
			<ul class="item_stats" style="margin:0;">
					...
					<li class="baths">%beds% '.SPEC($GLOBALS['_LANG']['_bedrooms']).'</li>
					<li class="beds">%baths% '.SPEC($GLOBALS['_LANG']['_bathrooms']).'</li>
				</ul>        
...
print  '<img src="'.$PPT->ImageCheck(trim($_GET['d']),"full").'" style="max-width:600px;max-height:450px;">';
	...
	include_once("../../admin/importtools/amazon_api_class.php");
