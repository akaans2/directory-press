...
class PremiumPressTheme_Datafeedr {
	...
	global $PPT,$wpdb; $AddtoCats = array(); $AddCounter=0; $UpdateCounter=0; $sc=array(); $PARENTCAT="";
	...
	while ($cats = mysql_fetch_array($result)) {
		...
		if($cats['parent'] == 0){
			...
			$a = $this->wp_create_category1($cats['name'], $PARENTCAT);
		...
		$RUN_QUERY = "SELECT ".$wpdb->prefix."dfr_shop_products.*, 
		".$wpdb->prefix."dfr_shop_cats.name AS categoryName FROM ".$wpdb->prefix."dfr_shop_products 
		INNER JOIN ".$wpdb->prefix."dfr_shop_cats ON (".$wpdb->prefix."dfr_shop_cats.id = ".$wpdb->prefix."dfr_shop_products.cat_id )";
