...
/* ------------------------------------------------------------------------------- */
	...
	function query_orderby($sort) {
        ...
        if ( (is_tag() OR is_category() OR is_author() OR is_date() OR is_home()) ) {
           ...
           if (in_array($_GET['quick'],$array_of_numeric_fields) && $ee[0] =="wp_postmeta.meta_value" ) {
...
/* ------------------------------------------------------------------------------- */
	...
	function Pages($type=0){
		...
		$STRING.= '<li class="'.$xbit.'"><a href="'.get_page_link( $Page->ID ).'" title="'.$Page->post_title.'">';
	...
	function Categories($ONLYID=0,$STYLE="",$desc=false, $dropnav=false, $onlyshowtickscats=true){
			...
			$STOPSHOWLIMIT = 200;
			if(PREMIUMPRESS_SYSTEM == "CouponPress"){ $STOPSHOWLIMIT = 14; }
			
 
			$STRING = ""; $DISPLAYCATS = ""; $MAKEDISPLAY = "";  $SAVED_DISPLAY = get_option("nav_cats"); $DONECATS = array(); $addToEnd ="";
			...
			if(!$onlyshowtickscats){ $MAKEDISPLAY =""; $dd = explode(",",str_replace("-","",get_option('article_cats'))); $DONECATS = $dd; }		
 
	 		$Maincategories = get_categories('pad_counts=1&use_desc_for_title=1&hide_empty=0&parent=0&hierarchical=0'.$MAKEDISPLAY);
			...
			foreach($SHOWRESULTS as $ThisCat){  if( ( isset($ThisCat['ID']) && $ThisCat['ID'] > 0 && $MAKEDISPLAY != "" ) || ( $MAKEDISPLAY =="" ) ){
				...
				if( strlen($Maincat->cat_name) > 1 && ($MAKEDISPLAY =="" && !in_array($Maincat->term_id,$DONECATS) ) || ( isset($ThisCat['ID']) && $Maincat->term_id == $ThisCat['ID'] ) ) {

				array_push($DONECATS, $Maincat->term_id); 

				$sub_categories = get_categories('parent='.$Maincat->cat_ID.'&depth=1&hide_empty=0&hierarchical=0'); 
				$count_sub = count($sub_categories);	

				if($count_sub > $STOPSHOWLIMIT && $desc && $dropnav){ 
					$ParentLink = 'href="#" onclick="toggle_slide(\'car_'.$Maincat->term_id.'\'); return false;"'; 
				}else{ 
					$ParentLink = 'href="'.get_category_link( $Maincat->term_id ).'"';
				}
	 
				$STRING .= '<li'.$STYLE.'><a '.$ParentLink.' title="'.$Maincat->category_nicename.'">';
				$STRING .= $Maincat->cat_name;
				//if($desc){ $STRING .= '<em>'.$Maincat->description.'</em>'; }
				$STRING .= '</a>';

	
				if( count($count_sub) > 0 ){

				if($ONLYID > 0 && $ONLYID != $Maincat->term_id){
				
				}else{

				if($count_sub > $STOPSHOWLIMIT && $dropnav){
 
					$addToEnd .= '</ul><div class="adv_categories" id="car_'.$Maincat->term_id.'">';
					$in=1;
					foreach ($sub_categories as $cat) {	
						if($in == 1){ $addToEnd .="<div class='catcol'><ul>";}							
						$addToEnd .= '<li class="cat-item"><a href="'.get_category_link( $cat->term_id ).'">';
						$addToEnd .= $cat->cat_name;
						$addToEnd .= '</a></li>';
						$in++;
						if($in == 8){ $addToEnd .="</ul></div>"; $in=1;  }
					}

					if($in > 1 && $in < 8){ $addToEnd .="</ul></div>";}
					$addToEnd .="</div></ul>";
				...
				$STRING .="<ul>";					
				foreach ($sub_categories as $cat) {						
					$STRING .= '<li><a href="'.get_category_link( $cat->term_id ).'">';
					$STRING .= $cat->cat_name;
					$STRING .= '</a>';						 				 				
								$sub_sub_categories	= get_categories('parent='.$cat->cat_ID.'&depth=1&hide_empty=0');							 
								$count_sub_sub 		= count($sub_sub_categories);				

								if($count_sub_sub > 0){

									$STRING .="<ul>";							
									foreach ($sub_sub_categories as $cat1) {	
										$STRING .= '<li><a href="'.get_category_link( $cat1->term_id ).'">';
			...
			if(!$onlyshowtickscats){ echo $STRING; return; }
			
		}

		echo $STRING.$addToEnd;	

		if(get_option("display_showpages") =="next"){
	...
	function ImageCheck($img,$ex="thumb"){
	...
	global $wpdb;
		...
		if($ex == "full" || !isset($GLOBALS['premiumpress']['thumbresize']) || $GLOBALS['premiumpress']['thumbresize'] == "off"){ $extra = ""; }else{ $extra= "_tbs.php?src="; } 
			...
			if($ex == "full" || $GLOBALS['premiumpress']['thumbresize'] == "off"){ return $rStr ."?"; }else{ return $rStr; }
		}
	}


	function Image($data,$type="m"){
	...
	function CategoryList($id=0,$showAll=false,$showExtraPrice=false,$TaxType=""){
		...
		$catlist="";
 		$Maincategories = get_categories('pad_counts=1&use_desc_for_title=1&hide_empty=0'.$TaxType.'&hierarchical=0'.$extra); 
...
/************ SHRINK THE WEB FUNCTIONALITY ******************/
	...
	function SaveImage($url, $args=""){
		...
		if($_SERVER['HTTP_HOST'] == "localhost"){
			...
			$query_string="";
			$query_string.="&ip=115.44.186.82".//$_SERVER['SERVER_ADDR']; 
			$query_string.="&host=home".//$_SERVER['HTTP_HOST'];	
	...
	function CategoryExtras($cat_id,$type="text",$return=0){
		...
		if($type == "text"){
			...
			$img = $this->ImageCheck(get_option("cat_extra_image_".$cat_id));
...
// CURRENCY OPTIONS FOR MULTIPLE CURRENCY VALUES
	...
	if($ignore_credit ==0){ if(!isset($userdata->ID) || $userdata->ID ==""){
 	...
 	if($ignore_credit ==1){ $price=0; } // used for 
