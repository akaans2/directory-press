			...
			return this.each(
				function()
				{
					...
					if (!alreadyExists && s.createButton) {
						// create it!
						controller.button = jQuery('<a href="#" class="dp-choose-date" title="' + $.dpText.TEXT_CHOOSE_DATE + '">' + $.dpText.TEXT_CHOOSE_DATE + '</a>')
								.bind(
									'click',
									function()
									{
 ...
 *
 * @example jQuery('#date-picker').datePicker();
 * jQuery('#date-picker').dpSetRenderCallback(function($td, thisDate, month, year)
 * {
 ...
 *
 * @example jQuery('#date-picker').datePicker();
 * jQuery('#date-picker').dpSetPosition($.dpConst.POS_BOTTOM, $.dpConst.POS_RIGHT);
	...
	$.extend(
		...
		{	
			...
			setSelected : function(d, v, moveToMonth, dispatchEvents)
			{
				...
				var $td;
				jQuery(selectorString, this.context).each(
					function()
					{
						if (jQuery(this).data('datePickerDate') == d.asString()) {
							$td = jQuery(this);
							if (s.selectWeek)
							{
				...
				jQuery('td', this.context).not('.selected')[this.selectMultiple &&  this.numSelected == this.numSelectable ? 'addClass' : 'removeClass']('unselectable');
			...
			clearSelected : function()
			{
				this.selectedDates = {};
				this.numSelected = 0;
				jQuery('td.selected', this.context).removeClass('selected').parent().removeClass('selectedWeek');
			},
			display : function(eleAlignTo)
			{
				...
				if (c.inline) {
					$createIn = jQuery(this.ele);
					attrs = {
						'id'		:	'calendar-' + this.ele._dpId,
						'class'	:	'dp-popup dp-popup-inline'
					};

					jQuery('.dp-popup', $createIn).remove();
					cssRules = {
					};
				} else {
					$createIn = jQuery('body');
					attrs = {
						'id'		:	'dp-popup',
						'class'	:	'dp-popup'
					};
				...
				$createIn
					.append(
						jQuery('<div></div>')
							...
							.css(cssRules)
							.append(
//								jQuery('<a href="#" class="selecteee">aaa</a>'),
								jQuery('<h2></h2>'),
								jQuery('<div class="dp-nav-prev"></div>')
									.append(
										jQuery('<a class="dp-nav-prev-year" href="#" title="' + $.dpText.TEXT_PREV_YEAR + '">&lt;&lt;</a>')
											.bind(
												'click',
												function()
												{
										...
										jQuery('<a class="dp-nav-prev-month" href="#" title="' + $.dpText.TEXT_PREV_MONTH + '">&lt;</a>')
											.bind(
												'click',
												function()
												{
								...
								jQuery('<div class="dp-nav-next"></div>')
									.append(
										jQuery('<a class="dp-nav-next-year" href="#" title="' + $.dpText.TEXT_NEXT_YEAR + '">&gt;&gt;</a>')
											.bind(
												'click',
												function()
												{
										...
										jQuery('<a class="dp-nav-next-month" href="#" title="' + $.dpText.TEXT_NEXT_MONTH + '">&gt;</a>')
											.bind(
												'click',
												function()
												{
								...
								jQuery('<div class="dp-calendar"></div>')
							)
							.bgIframe()
						);
					
				var $pop = this.inline ? jQuery('.dp-popup', this.context) : jQuery('#dp-popup');
				...
				if (this.displayClose) {
					$pop.append(
						jQuery('<a href="#" id="dp-close">' + $.dpText.TEXT_CLOSE + '</a>')
							.bind(
								'click',
								function()
								{
...
//					jQuery('.selectee', this.context).focus();
					jQuery(document).bind('mousedown.datepicker', this._checkMouse);
				}
			...
			{
				// set the title...
				jQuery('h2', this.context).html((new Date(this.displayedYear, this.displayedMonth, 1)).asString($.dpText.HEADER_FORMAT));
			...
			_closeCalendar : function(programatic, ele)
			{
				if (!ele || ele == this.ele)
				{
					jQuery(document).unbind('mousedown.datepicker');
					jQuery(document).unbind('keydown.datepicker');
					this._clearCalendar();
