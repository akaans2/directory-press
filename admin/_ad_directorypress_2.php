...
<h3><img src="<?php echo PPT_FW_IMG_URI; ?>/admin/_ad_submission.png" align="middle"> Submit Page Setup</h3> <a class="premiumpress_button" href="javascript:void(0);" onclick="PPHelpMe()">Help Me</a>							 
<ul>
	...
	<tr class="mainrow"><td></td><td class="forminp">
		...
		<select name="adminArray[tpl_add_mustlogin]" style="width: 240px;  font-size:14px;">
           ...
           <option value="no" <?php if(get_option("tpl_add_mustlogin") =="no"){ print "selected";} ?>>No</option>
			...
			<p class="ppnote">This will stop non registered users from submitting listings.</p>
		...
		<select name="adminArray[display_listing_status]" style="width: 240px;  font-size:14px;">
			...
			<p class="ppnote">Select the default status of the link when its submitted.</p>
...
<small>Display the country selection box on the submit page.</small>
	...
	<select name="adminArray[display_fileupload]" style="width: 240px;  font-size:14px;">
            ...
            <p class="ppnote">Enable/Disable file uploads.</p> 
...
<p class="ppnote1">Enter a numeric value (1,2,3 etc) for the maximum file uploads per post. </p>
            <?php } ?>
 ...
 <p> <b>Require reciprocal link</b></p>   
            
            		<select name="adminArray[display_rlink]" style="width: 240px;  font-size:14px;">
<option value="yes" <?php if(get_option("display_rlink") =="yes"){ print "selected";} ?> >Yes</option>
<option value="no" <?php if(get_option("display_rlink") =="no"){ print "selected";} ?>>No</option>
</select>
<p class="ppnote">This will require the link submitter to put your link on their website before submitting.</p>


<p><b>Reciprocal link Code</b></p>
<textarea name="adminArray[display_rlink_text]" cols="" rows="" style="width: 240px;  font-size:14px;"><?php echo stripslashes(get_option("display_rlink_text")); ?></textarea>
 
<p class="ppnote">This is the code they must put on their website before submitting to your directory.</p>
...
<div id="PPT-catpricebox"></div>
	 ...
	 <p class="ppnote"> Here you can setup additional prices per category.</p>
...
<table class="maintable">
	...
	<tr class="mainrow">
	  <td class="titledesc">Enable Packages</td>
		<td class="forminp">

			<input type="checkbox" name="pak_enabled" value="1" <?php if(get_option("pak_enabled") ==1){ echo "checked"; } ?>><p class="ppnote">Enable this to allow custom pricing on your submit listing page.</p>
...
<tr>
	...
	<tr class="mainrow">
		<td class="titledesc">Sidebar Help Text</td>
		<td class="forminp">
<textarea name="adminArray[pak_help_text]" cols="" rows="" style="width: 400px;  font-size:14px;"><?php echo stripslashes(get_option("pak_help_text")); ?></textarea>
...
<p class="ppnote">This is the text that appears on the sidebar whilst users add new listings. You can include HTML.</p>
	...
	<tr class="mainrow">
		<td class="titledesc">Bottom Text</td>
		<td class="forminp">
<textarea name="adminArray[pak_text]" cols="" rows="" style="width: 400px;  font-size:14px;"><?php echo stripslashes(get_option("pak_text")); ?></textarea>
...
<center><img src="<?php echo $GLOBALS['template_url']; ?>/template_directorypress/images/help1/a26.png"></center>
...
<a href="#" onclick="toggleLayer('package<?php echo $i; ?>');">
...
<td  style="background:#fff;">
	<tr class="mainrow">
	  <td class="titledesc">Enable Package</td>
		<td class="forminp">
	...
	<tr class="mainrow" style="background:#fff;">
	  <td class="titledesc">Package Name</td>
		<td class="forminp">
...
<tr><td>
	...
	<tr class="mainrow" style="background:#fff;">
	  <td class="titledesc">Package Price</td>
		<td class="forminp">
	...
	<tr class="mainrow"  style="background:#fff;">
	  <td class="titledesc">Days Before Expiry</td>
		<td class="forminp">
			<input type="text" name="package[<?php echo $i; ?>][expire]" style="width:50px;" value="<?php echo $packdata[$i]['expire']; ?>"><p class="ppnote">Give a numeric value for amount of time the listing will display before its automatically removed. Leave blank for never.</p>
		</td>
	</tr>
...
<table width="100%"  border="0">
  ...
  <tr>
    <td><input type="checkbox" name="package[<?php echo $i; ?>][a5]" value="1" <?php if(isset($packdata[$i]['a5']) && $packdata[$i]['a5'] ==1){ echo "checked"; } ?>> <strong>Featured Listing</strong><br /><p>This will highlight the users listing in the website searches.</p></td>
    <td><input type="checkbox" name="package[<?php echo $i; ?>][a6]" value="1" <?php if(isset($packdata[$i]['a6']) && $packdata[$i]['a6'] ==1){ echo "checked"; } ?>> <strong>Remove Reciprocal Link</strong><br />
    <p>This will remove the reciprocal link requirement for this package.</p>
    </tr>
...
<p><b> Custom Field ID<b></p> <input name="customfield[<?php echo $i; ?>][key]" type="text" style="width: 230px; font-size:14px;" class="txt" value="<?php if(isset($cf1[$i]['key']) && strlen($cf1[$i]['key']) > 1){ echo $cf1[$i]['key']; }else{ print "Key_".$i;} ?>" /><br /><small>Example: key_<?php echo $i;?> </small>
       <img src="<?php echo $GLOBALS['template_url']; ?>/images/premiumpress/h-ico/help.png" onmouseover="this.style.cursor='pointer';" 
...
<p><b>Package Page Description</b></p>
 <input name="customfield[<?php echo $i; ?>][desc1]" type="text" style="width: 230px; font-size:14px;" class="txt" value="<?php if(isset($cf1[$i]['desc1'])){ echo $cf1[$i]['desc1']; } ?>" /> 
 <small>Explain a little bit about what the field values are for.</small>
