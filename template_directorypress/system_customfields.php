...
function premiumpress_customfields_box() {
	...
	add_meta_box( 'premiumpress_customfields_0', __( 'DirectoryPress Listing Options', 'sp' ), 'premiumpress_directorypress', 'post', 'normal', 'high' );
...
function premiumpress_directorypress(){
...
<?php	
    function generate_select($values, $selected, $usekey = '') {
        foreach ($values as $key => $data) {
            ...
            if ($selected == $option_value) {
                echo "<option value = '$option_value' selected>$data</option>";
            } else {
                echo "<option value = '$option_value'>$data</option>";
            }
...
<h3 style="background:none;font-size:18px;margin-left:0px;padding-left:0px;"><img src="../wp-content/themes/directorypress/images/premiumpress/h-ico/open alt.png" align="middle"> Options</h3>	 <a class="premiumpress_button" href="javascript:void(0);" onclick="PPHelpMe(\'addlisting\')">Help Me</a>						 
<ul>
	<li><a rel="premiumpress_tab1" href="#" class="active"> Data</a></li>
	<li><a rel="premiumpress_tab2" href="#">Custom Fields</a></li>
    <li><a rel="premiumpress_tab3" href="#">Images</a></li>
    <li><a rel="premiumpress_tab4" href="#">Packages</a></li>
...
<div id="premiumpress_tab1" class="content">';
	...
	echo '<table width="100%"  border="0"><tr width="50%"><td valign="top">';
		...
		echo '<label style="font-size:14px; line-height:30px;">' . __("<b>Featured Website</b> <small>(highlighted in search results)</small>", 'sp' ) . '</label><br />';
...
//		<option value="no" '.$a2.'>no</option></select><br /><br />';
        generate_select($search_priority, $fea, 1);
        echo '</select><br /><br />';
...
<option value="Zimbabwe">Zimbabwe</option> 
	...
	echo '<table width="100%"  border="0" style="background:#ffdec7; padding:10px;"><tr width="50%"><td valign="top">';

	echo '<label style="font-size:14px; line-height:30px;">' . __("<b>Website Address</b> (http://..)", 'sp' ) . '</label><br />';
	...
	echo '<label style="font-size:14px; line-height:30px;">' . __("<b>Affiliate Link *optional*</b> <br><small>will change the clickable link to the one you enter here</small>", 'sp' ) . '</label><br />';
	...
	echo '<label style="font-size:14px; line-height:30px;">' . __("<b>Hits/View Counter</b> ", 'sp' ) . '</label><br />';
		...
		//echo '<label style="font-size:14px; line-height:30px;">' . __("<b>Bid Value</b> (used only for 'link bid' setups) ", 'sp' ) . '</label><br />';
		//echo '<input type="text" name="field[bid]" value="'.get_post_meta($post->ID, 'bid', true).'" style="font-size:14px;padding:5px; width:95%; background:#e7fff3;" /><br /><br />';
...
<div id="premiumpress_tab2" class="content">';
	...
	for($i=1; $i < 20; $i++){ if(isset($cf1[$i]['enable']) && $cf1[$i]['enable'] ==1){
...
<div id="premiumpress_tab3" class="content">';
	...
	echo '<div id="DisplayImages" style="display:none;"></div><input type="hidden" id="searchBox1" name="searchBox1" value="" /> <table width="100%"  border="0"><tr width="50%"><td valign="top">';
		...
		?>
		<a href='javascript:void(0);' onClick="toggleLayer('DisplayImages'); add_image_next(0,'<?php echo get_option("imagestorage_path"); ?>','<?php echo get_option("imagestorage_link"); ?>','g_featured_image');"><img src="<?php echo $GLOBALS['template_url']; ?>/images/premiumpress/led-ico/find.png" align="middle"> View Uploaded Images</a>  
        
        
        <input id="upload_g_featured_image" type="button" size="36" name="upload_g_featured_image" value="Upload Image" />
	...
	echo '</td><td width="50%" valign="top">';
		...
		echo '<label style="font-size:14px; line-height:30px;">' . __("<b>Gallery Images</b> <br /><small>(seperate multiple images with a comma)</small>", 'sp' ) . '</label><br />';
...
<div id="premiumpress_tab4" class="content">';
	...
	echo '<table width="100%"  border="0"><tr width="50%"><td valign="top">';
		...
		echo '<label style="font-size:14px; line-height:30px;">' . __("<b>Purchase Price</b> ", 'sp' ) . '</label><br />';
...
echo '<link rel="stylesheet" type="text/css" href="'.PPT_PATH.'js/lightbox/themes/default/jquery.lightbox.css" />
        <!--[if IE 6]><link rel="stylesheet" type="text/css" href="'.PPT_PATH.'js/lightbox/themes/default/jquery.lightbox.ie6.css" /><![endif]-->    
        <script type="text/javascript" src="'.PPT_PATH.'js/lightbox/jquery.lightbox.min.js"></script>   
...
function premiumpress_columns($defaults) {
	...
	if(isset($_GET['post_type']) && $_GET['post_type'] == "article_type"){
...
function premiumpress_custom_column($column_name, $post_id) {
	...
	} else if( $column_name == 'category' ) {   //Adding 2nd column
