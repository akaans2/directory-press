    ...
    MsgBoxObject: {
      ...
      overlay : {
        ...
        resize: function(x, y) {
          ...
          var win = { x: jQuery(document).width(), y: jQuery(document).height() };
      ...
      create: function() {
        ...
        this.esqueleto.form = jQuery('<form action="'+this.options.formaction+'" method="post"></form>');
      ...
      addevents: function() {
        jQuery(window).bind('resize', $.proxy(function() {
          if (this.visible)
          {
            this.overlay.resize();
            this.moveBox();
        ...
        jQuery(window).bind('scroll', $.proxy(function() {
          if (this.visible)
          {
            this.moveBox();
          }
        ...
        this.esqueleto.form.bind('submit', $.proxy(function(event) {
          jQuery('input[type=submit]:first, button[type=submit]:first, button:first', this.esqueleto.form).trigger('click');
        ...
        this.overlay.element.bind('show', $.proxy(function() { jQuery(this).triggerHandler('show'); }, this));
        this.overlay.element.bind('hide', $.proxy(function() { jQuery(this).triggerHandler('close'); }, this));
        ...
        this.esqueleto.msgbox.queue(this.options.name, $.proxy(function( next ) {
          ...
          if (inputs !== undefined)
          {
            ...
            $.each(inputs, $.proxy(function(i, input) {
              if (input.type == 'text' || input.type == 'password')
              {
                ...
                this.esqueleto.inputs.append(jQuery(iLabel+'<input type="'+input.type+'" name="'+iName+'" value="'+input.value+'" autocomplete="off" '+iRequired+'/>'+fLabel));
              }
              else if (input.type == 'checkbox')
              {
                ...
                iName  = input.name === undefined ? this.options.name+'-label-'+i : input.name;
                this.esqueleto.inputs.append(jQuery(iLabel+'<input type="'+input.type+'" style="display:inline; width:auto;" name="'+iName+'" value="'+input.value+'" autocomplete="off"/> '+fLabel));
              }
            }, this));
          }
          ...
          if (options.type == 'alert' || options.type == 'info' || options.type == 'error' || options.type == 'confirm')
          {
            $.each(buttons, $.proxy(function(i, button) {
              ...
              {
                this.esqueleto.buttons.append(jQuery('<button type="submit">'+button.value+'</button>').bind('click', $.proxy(function(e) { this.close(button.value); e.preventDefault(); }, this)));
              }
              else if (button.type == 'cancel')
              {
                this.esqueleto.buttons.append(jQuery('<button type="button">'+button.value+'</button>').bind('click', $.proxy(function(e) { this.close(false); e.preventDefault(); }, this)));
              }
            }, this));
          }
          else if (options.type == 'prompt')
          {
            $.each(buttons, $.proxy(function(i, button) {
              if (button.type == 'submit')
              {
                this.esqueleto.buttons.append(jQuery('<button type="submit">'+button.value+'</button>').bind('click', $.proxy(function(e) {
          ...
          setTimeout($.proxy(function() { var b = jQuery('input, button', this.esqueleto.msgbox); if (b.length) { b.get(0).focus();} }, this), this.options.moveDuration);
        }, this));
      ...
      moveBox: function() {
        var size   = { x: jQuery(window).width(),      y: jQuery(window).height() };
        var scroll = { x: jQuery(window).scrollLeft(), y: jQuery(window).scrollTop() };
