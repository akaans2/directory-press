...
<?php if(isset($data) && get_option('pak_enabled') ==1 ){ ?>
 ...
 <?php } ?>       
        ...
        <p class="f_half left"> 
            ...
            <input type="text" name="form[email]"  class="short" tabindex="9" id="email1" value="<?php if(isset($_POST['action']) && !isset($data) ){ echo $_POST['form']['email']; }elseif(isset($data)){ echo $data['email']; 
        ...
        <p class="f_half left"> 
            <label for="name"><?php echo SPEC($GLOBALS['_LANG']['_tpl_add20']) ?><span class="required">*</span></label><br />
       ...
       <?php if(get_option('display_rlink') =="yes"  && $PACKAGE_OPTIONS[$_POST['packageID']]['a6'] != 1 ){ ?>
       ...
       <p class="f_half left"> 
        ...
        <?php } if($PACKAGE_OPTIONS[$_POST['packageID']]['a6'] == 1){ print "<input type='hidden' name='norec' value='1'>"; } ?>    
...
<div class="full clearfix border_t box"> 
   ...
   <?php $i = 0; if(is_array($_POST['custom'])){  foreach($_POST['custom'] as $cus){ if($cus['value'] != ""){ ?>
    ...
    <div class="full clearfix border_t box">  
	  ...
	  <div class="f4 left"><p><?php echo $cus['value']; ?>&nbsp;</p></div>      
    </div>   
  	<input type="hidden" name="custom[<?php echo $i; ?>][name]" value="<?php echo $cus['name']; ?>" />
    <input type="hidden" name="custom[<?php echo $i; ?>][value]" value="<?php echo PPTCLEAN($cus['value']); ?>" />
   ...
   <?php } $i++; } }  ?>
    ...
    <?php } } ?> 
    	...
    	<?php  if( get_option('display_rlink') =="yes" && isset($GLOBALS['recpFound']) || get_option('display_rlink') =="no" || $PACKAGE_OPTIONS[$_POST['packageID']]['a6'] == 1){  ?>
