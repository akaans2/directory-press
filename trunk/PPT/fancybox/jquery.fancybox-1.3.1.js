...
(function($) {
	...
	var tmp, loading, overlay, wrap, outer, inner, close, nav_left, nav_right,
		...
		start_pos, final_pos, busy = false, shadow = 20, fx = $.extend(jQuery('<div/>')[0], { prop: 0 }), titleh = 0, 
		...
		_finish = function () {
			...
			jQuery(window).bind("resize.fb", $.fancybox.center);

			if (currentOpts.centerOnScroll) {
				jQuery(window).bind("scroll.fb", $.fancybox.center);
			} else {
		...
		fancybox_start = function() {
			...
			selectedOpts = $.extend({}, $.fn.fancybox.defaults, (typeof jQuery(obj).data('fancybox') == 'undefined' ? selectedOpts : jQuery(obj).data('fancybox')));
	...
	$.fn.fancybox = function(options) {
		jQuery(this)
			.data('fancybox', $.extend({}, options, ($.metadata ? jQuery(this).metadata() : {})))
		...
		if ($.isArray(obj)) {
			for (var i = 0, j = obj.length; i < j; i++) {
				if (typeof obj[i] == 'object') {
					jQuery(obj[i]).data('fancybox', $.extend({}, opts, obj[i]));
				} else {
					obj[i] = jQuery({}).data('fancybox', $.extend({content : obj[i]}, opts));
				}
			}
			...
			if (typeof obj == 'object') {
				jQuery(obj).data('fancybox', $.extend({}, opts, obj));
			} else {
				obj = jQuery({}).data('fancybox', $.extend({content : obj}, opts));
			}
