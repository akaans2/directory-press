		...
		function fetch_rates() {
        	...
        	if ($status == '1') {
        		...
        		for($i=0; $i<$num_of_rates; $i++) {
        			...
        			if ($this->default_rate == $service_code) $selected = ' selected';
        			$option_list .= '<option value="'.$service_cost.'"'.$selected.'>'.$service_desc.' - '.$this->currency_type.$service_cost.'</option>';
        		...
        		if ($this->debug == true) return  'UPS returned this error: "<strong>'.$xml->Response->Error->ErrorDescription.'</strong>"<br><br>Please check the readme.txt file for more information or contact UPS directly. ';
        	...
        	return '<select name="shipping_rates" onchange="javascript:document.'.$this->form_name.'.submit();"'.$class.'>'.$option_list.'</select>';
