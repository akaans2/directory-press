...
<?php $i=0; while($i < 1000){ if($val2 == $i){ $ex ="selected=selected"; }else{ $ex ="";  } ?><option value="<?php echo $i; ?>" <?php echo $ex; ?>><?php echo $i; ?></option><?php $i=$i+50; } ?>
</select> 

<p><b>Maximum Amount: 0 - <u>100</u></b></p>
<select name="ppt_control_4_widget_high" style="width: 220px;  font-size:14px;">
<?php $i=0; while($i < 15000){ if($val1 == $i){ $ex ="selected=selected"; }else{ $ex ="";  } ?><option value="<?php echo $i; ?>" <?php echo $ex; ?>><?php echo $i; ?></option><?php $i=$i+500; } ?>
</select> 
 

<p><b>Price Range:</b></p>
<select name="ppt_control_4_widget_range" style="width: 220px;  font-size:14px;">
<?php $i=100; while($i < 1100){ if($val3 == $i){ $ex ="selected=selected"; }else{ $ex ="";  } ?><option value="<?php echo $i; ?>" <?php echo $ex; ?>><?php echo $i; ?></option><?php $i=$i+50; } ?>
</select> 
...
function ppt_widget_3_display($args){ 
	...
	if( $PPTDesign->CanDisplayElement(get_option("display_categories_pages"))  ){ 
			...
			$STRING .= '<ul class="category" id="Accordion">'. $PPTDesign->SideBarNavigationBox() .'</ul><div class="clearfix"></div>';
...
<input class="widefat" type="text" id="ppt_control_3_widget_title" name="ppt_control_3_widget_title" value="<?php echo get_option("ppt_control_3_widget_title"); ?>" />
		...
		<p><b>Display Sub Categories</b> (Tabbed)</p>			
			<select name="display_tabbed_cats" style="width: 220px;  font-size:14px;">
