...
class PPT extends PPT_API {
	...
	global $post, $pagenow, $page;
		...
		if($currentPage == "front_page"){	 
			...
			echo "<script type='text/javascript' src='".PPT_THEME_URI."/PPT/js/jquery.siteFeature.js'></script>";
			echo "<link rel='stylesheet' type='text/css' href='".PPT_THEME_URI."/PPT/css/css.fslider.css' media='screen' />";
			} 
		...
		// Load lightbox into the single page or all pages for couponpress	
			...
			echo "<link rel='stylesheet' type='text/css' href='".PPT_THEME_URI."/PPT/js/lightbox/themes/default/jquery.lightbox.css' media='screen' />";
			echo "<script type='text/javascript' src='".PPT_THEME_URI."/PPT/js/lightbox/jquery.lightbox.min.js'></script>";	 	
			echo "<!--[if IE 6]><link rel='stylesheet' type='text/css' href=".PPT_PATH."js/lightbox/themes/default/jquery.lightbox.ie6.css' /><![endif]--> ";
 
        }  
		
		// Load lightbox into couponpress
		if(strtolower(PREMIUMPRESS_SYSTEM) == "couponpress" && !isset($_GET['action']) && !isset($_GET['redirect_to']) &&  $currentPage != "" ){
	...
	case "directorypress": { 
       ...
       <?php   if($f[0] =="singular" && get_option('display_themecolumns') =="3"){ ?> <style> #PPTSinglePage-post .rightSidebar {width: 315px;}</style><?php } ?>
		...
		echo stripslashes(get_option("google_webmaster_code"));
 		...
 		<!--[if IE 6]><link rel="stylesheet" href="<?php echo PPT_CHILD_URL; ?>stylesIE6.css" type="text/css" media="screen" /><![endif]-->
	...
	global $pagenow;
		...
		<script type="text/javascript" src="<?php echo PPT_FW_JS_URI; ?>validation.js"></script>
		...
		if(is_single() || ( strtolower(PREMIUMPRESS_SYSTEM) == "couponpress" && !isset($GLOBALS['tpl-add']) && !isset($_GET['action']) && !isset($_GET['redirect_to']) && $pagenow !="wp-login.php" ) ){ ?>
 		<script type="text/javascript">
	...
	register_post_type( 'faq_type',
		array(
		  ...
		  'public' => true,
		  'supports' => array ( 'title', 'editor','author', 'revisions', 'post-formats', 'trackbacks', 'comments' ) 
		)
	  );
