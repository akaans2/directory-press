...
<div class="btn-join clearfix">&nbsp;&nbsp;</div>


<div class="heading-top"><h2>title here</h2></div>


<img src="<?php echo IMAGE_PATH; ?>banner-right.jpg" /> 



 <form method="get"  action="<?php echo $GLOBALS['bloginfo_url']; ?>/" name="searchBox" id="searchBox">
<input type="text" value="<?php echo SPEC($GLOBALS['_LANG']['_head1']) ?>" name="s" id="s" onfocus="this.value='';"/>
<select id="catsearch" name="cat"><option value="">---------</option><?php echo $PPT->CategoryList(); ?></select> 
<div class="searchBtn" onclick="document.searchBox.submit();"> &nbsp;</div>

<?php if(get_option("display_advanced_search") ==1){ ?><a href="javascript:void(0);" onClick="jQuery('#AdvancedSearchBox').show();"><small><?php echo SPEC($GLOBALS['_LANG']['_head2']) ?></small></a><?php } ?>
            </form>
